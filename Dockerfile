...
RUN bash -c "if [ \"z${LIBC}\" == \"zuClibc-ng\" ]; then wget --no-check-certificate https://downloads.uclibc-ng.org/releases/${LIBC_VER}/uClibc-ng-${LIBC_VER}.tar.gz; fi"
RUN wget --no-check-certificate http://ftpmirror.gnu.org/gmp/gmp-${GMP_VER}.tar.gz
RUN wget --no-check-certificate http://ftpmirror.gnu.org/mpfr/mpfr-${MPFR_VER}.tar.gz
RUN wget --no-check-certificate http://www.multiprecision.org/downloads/mpc-${MPC_VER}.tar.gz
